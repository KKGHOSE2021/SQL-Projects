...
-- In this task, retrieve data from tables as in task one. In addition, 
-- set conditions on the result set of a query using the WHERE clause
#############################
...
-- 4.5: Extract a list of all employees whose first name starts with 'Mar' and have four characters

SELECT *
FROM employees
WHERE first_name LIKE ('Mar_');

-- 4.6: Extract a list of all employees whose first name don't contain 'Mar'
...
-- In this task, retrieve distinct data from tables in the employees database.
-- In addition, sort the result set, how to limit the result set and how to
...
SELECT DISTINCT first_name COUNT(*) AS first_name_count
FROM employees
GROUP BY first_name
