...
-- Exercise 5.3: Number each customer by their customer segment
...
salary - LEAD(salary) OVER() AS salary_difference
FROM employees;
...
-- Exercise 6.2: Retrieve all employees first name, department, salary, closest_lowest_salary, 
-- next_closest_salary for clothing department
SELECT first_name, department, salary,
...
SELECT *, AGE(hire_date, first_emp_date) AS duration
FROM (
	SELECT first_name, department, hire_date
	FIRST_VALUE(hire_date) OVER(ORDER BY hire_date) AS first_emp_date
	FROM employees) a
...
-- Order by the first_name in ascending order
SELECT first_name, email, department, salary,
NTH_VALUE(salary, 5) OVER(PARTITION BY department ORDER BY first_name)
FROM employees;
